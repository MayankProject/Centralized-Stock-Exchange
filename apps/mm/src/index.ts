...
function makeOrder(): Promise<[string, string]> {
	...
	return new Promise((resolve, reject) => {
		axios.post<{ orderId: string }>("http://localhost:3000/order", payload).then((data: AxiosResponse<{ orderId: string }>) => resolve([data.data.orderId, user])).catch(() => { })
	})
}
...
function cancelOrder() {
	...
	return new Promise((resolve, reject) => {
		axios.delete("http://localhost:3000/order", {
			data: payload
		}).catch(() => { })
	})
}
